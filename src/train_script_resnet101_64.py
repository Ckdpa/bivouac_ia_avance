...
train_files, test_files, val_files = train_test_val_split_list(list_data_files('./process_image_output_64/'))

mask_as_int = True
batch_size = 64*4 # Multiply by 4 as we run on 4 GPUs
...
test_dataloader = DataLoader(LabelDataset(test_files, mask_as_int), batch_size=batch_size, shuffle=True, num_workers=2)
...
for epoch in range(1, n_epochs+1):
    ...
    with torch.no_grad():
        ...
        if val_loss < valid_loss_min:
            ...
            torch.save(model.state_dict(), './output/resnet_101_64_tcga.pt')
